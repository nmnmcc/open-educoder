...
const editContent = Effect.fn("assignments.labs.editContent")(function* (input: {
  ...
  if (editor === null) {
    return yield* failInput("Set $VISUAL or $EDITOR to edit assignment files.");
  }
...
export const Edit = Command.make(
  ...
  Effect.fn("assignments.labs.edit")(function* (input) {
    ...
    const contentResult = yield* labAssignmentFeature.getRepositoryContent({
    ...
    const saveResult = yield* labAssignmentFeature
      .saveRepositoryFile({
        courseId: input.courseId,
        path: input.path,
        homeworkId: input.homeworkId,
  ...
  Command.withExamples([
    ...
    {
      command: "VISUAL='code --wait' open-educoder assignments labs edit 109348 3487324 case1/code.sh",
