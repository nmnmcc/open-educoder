...
export const ShixunListPage = Effect.gen(function* () {
  const homeworkShixun = yield* HomeworkShixunFeature;

  return function ShixunListPage({ route, nav, ui, active }: PageProps<Extract<Route, { name: "shixunList" }>>) {
    const [page, setPage] = useState(1);
    const [search, setSearch] = useState("");
    const [refresh, setRefresh] = useState(0);
    const data = useRemoteData(`shixun:${route.courseId}:${page}:${search}:${refresh}`, () =>
      homeworkShixun.list({
    ...
    useInput(
      (input) => {
        ...
        if (input === "/") {
          void ui.prompt("Search shixun homeworks", "keyword", search).then((value) => {
            if (value !== null) {
...
function ShixunListContent({
  ...
  readonly route: Extract<Route, { name: "shixunList" }>;
  ...
  const [selected, setSelected] = useSelectedIndex(entries.length, `shixun-list:${page}:${search}:${refresh}`);
    ...
    <Box flexDirection="column">
      ...
      <Box marginTop={1}>
        <SelectList
          ...
          active={active}
          empty="No shixun homeworks found."
        />
      </Box>
    </Box>
  );
...
export const ShixunDetailPage = Effect.gen(function* () {
  const homeworkShixun = yield* HomeworkShixunFeature;

  return function ShixunDetailPage({ route, nav, ui, active }: PageProps<Extract<Route, { name: "shixunDetail" }>>) {
